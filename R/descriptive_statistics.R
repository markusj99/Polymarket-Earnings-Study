...
get_start_dir <- function() {
  # Works for Rscript --file=... and for interactive sessions
...
paths <- list(
  ...
  brier        = file.path(data_dir, "brier_scores", "brier_scores_market_horizon.csv"),
...
brier_raw        <- read_csv_required(paths$brier)
...
heck_comp_raw    <- read_csv_required(paths$heck_comp)
heck_events_raw  <- read_csv_required(paths$heck_events)
